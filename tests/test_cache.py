...
def _reset_api_state() -> None:
    run_id = uuid4().hex
    api_server._TRACK_DB_PATH = f"test_cache_{run_id}.db"
    api_server._VECTOR_INDEX_PATH = f"test_cache_{run_id}_index"
...
def test_search_cache_hit_miss_metrics():
    _reset_api_state()
    tmp_dir = Path(f"test_cache_{uuid4().hex}")
    ...
    try:
        ...
        query_path = tmp_dir / "q.wav"
        query_path.write_bytes(_make_wav_bytes(440.0))

        payload = api_server.SearchRequest(query_audio_path=str(query_path), top_k=3)
        api_server.search(payload)
        api_server.search(payload)

        metrics = api_server.cache_metrics()
