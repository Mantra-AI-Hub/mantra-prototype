...
def _reset_api_state() -> None:
    run_id = uuid4().hex
    api_server._TRACK_DB_PATH = f"test_search_api_{run_id}.db"
    api_server._VECTOR_INDEX_PATH = f"test_search_api_{run_id}_index"
...
def test_semantic_search_endpoint_returns_ranked_results():
    _reset_api_state()

    tmp_dir = Path(f"test_semantic_search_{uuid4().hex}")
    tmp_dir.mkdir(parents=True, exist_ok=True)

    try:
        indexed_audio = _make_wav_bytes(440.0)
        api_server.index_audio(
            audio_bytes=indexed_audio,
            filename="indexed.wav",
            track_id="semantic_track_1",
        )
        assert api_server._worker.process_one()

        query_path = tmp_dir / "query.wav"
        query_path.write_bytes(_make_wav_bytes(440.0))

        payload = api_server.SearchRequest(query_audio_path=str(query_path), top_k=5)
        response = api_server.search(payload)

        assert response["results"]
        assert response["results"][0]["track_id"] == "semantic_track_1"
