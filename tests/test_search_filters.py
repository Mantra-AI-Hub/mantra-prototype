...
def _reset_api_state() -> None:
    run_id = uuid4().hex
    api_server._TRACK_DB_PATH = f"test_search_filters_{run_id}.db"
    api_server._VECTOR_INDEX_PATH = f"test_search_filters_{run_id}_index"
...
def test_search_with_metadata_filters():
    ...
    tmp_dir = Path(f"test_search_filters_{uuid4().hex}")
    ...
    try:
        api_server.index_audio(
            ...
            track_id="ambient_track",
            genre="ambient",
            artist="unknown",
        )
        api_server.index_audio(
            audio_bytes=_make_wav_bytes(660.0),
            filename="rock.wav",
            track_id="rock_track",
            genre="rock",
            artist="band",
        ...
        payload = api_server.SearchRequest(
            query_audio_path=str(query_path),
            top_k=10,
            filters={"genre": "ambient", "artist": "unknown"},
        )
        response = api_server.search(payload)

        assert response["results"]
        assert response["results"][0]["track_id"] == "ambient_track"
        assert all(item["metadata"]["genre"] == "ambient" for item in response["results"])
    finally:
